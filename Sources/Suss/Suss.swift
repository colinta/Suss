...
func initial(_ initial: Model? = nil) -> () -> Initial<Model, Message> {
    if var initialModel = initial, !initialModel.url.isEmpty {
        do {
            initialModel.active = .responseBody
            let (model, commands) = try submit(model: &initialModel)
            return { Initial(model, commands) }
        } catch {
            initialModel.error = (error as? Error)?.description
            return { Initial(initialModel) }
        }
    } else {
